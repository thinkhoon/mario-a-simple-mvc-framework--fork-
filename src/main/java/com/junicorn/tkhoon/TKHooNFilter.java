...
import com.junicorn.tkhoon.route.Route;
import com.junicorn.tkhoon.route.RouteMatcher;
...
import com.junicorn.tkhoon.util.PathUtil;
import com.junicorn.tkhoon.util.ReflectUtil;
 ...
 *
 */
public class TKHooNFilter implements Filter {
	
	private static final Logger LOGGER = Logger.getLogger(TKHooNFilter.class.getName());
	...
	public void init(FilterConfig filterConfig) throws ServletException {
		...
		if(!tKHooN.isInit()){
			...
			bootstrap.init(tKHooN);
			
			Routers routers = tKHooN.getRouters();
			if(null != routers){
	...
	private void handle(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse, Route route){
		...
		TKHooNContext.initContext(servletContext, request, response);
